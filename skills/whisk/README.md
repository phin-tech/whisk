...
Concrete addresses are `pty:<id>`, `run:<id>`, `session:<id>`, `work-item:<id>`, and `project:<id>`. `send` also accepts `@project:<id>` and `@work-item:<id>` selectors, which the daemon expands to current concrete session, PTY, and run recipients before storage; `@idle` is deferred until agent status exists. `send` and `reply` default `-from` to `WHISK_PTY_ID`, then `WHISK_RUN_ID`, then `WHISK_SESSION_ID`; `check` defaults `-to` to every current address exposed in the Whisk environment.
